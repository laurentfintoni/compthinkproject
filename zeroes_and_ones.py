...
def process_citations(citations_file_path):
    ...
    matrix = []
    # empty dictionary for dynamic programming 
    doi_date_dict = {}
    ...
    with open(citations_file_path, mode='r') as file:
        ...
        for row in csvFile:
            matrix.append(row)
            # add two extra-column for with empty value to be filled by using dynamic programming approach
            row.update([('citing_year', ''), ('cited_year', '')])
            # initiliaze a value to check for citing year in dict, if not present, add it 
            ...
            # same thing for cited year 
            cited_doi = row['cited']
            if cited_doi in doi_date_dict:
                row['cited_year'] = doi_date_dict[cited_doi]
            else:
                timespan_str = row['timespan']
                # add -01 if date is not MM-DD-YYYY
                while len(citing_date) < 10:
                    citing_date = citing_date + '-01'
                # transform the string into a date object using ISO formatting 
                citing_n_date = date.fromisoformat(citing_date) 
                # split the timespan string in a list of values for Y, M, D and assign them to a relativedelta
                timespan_n = (re.split('[P|Y|M|D]', timespan_str)[1:-1]) 
                ...
                # take into consideration negative timespan 
                if timespan_str[0] == "-":  # just in case there is a negative timespan
                    delta_n = delta_n*-1                
                #calculate the cited date, transform to string, take first 4 characters
                cited_n_date = citing_n_date - delta_n
                cited_year = cited_n_date.strftime('%Y') 
...
def do_compute_impact_factor(data, dois, year):
    ...
    citation_results = []
    # create set to hold found and not found DOIs 
    dois_cited = set()
    dois_not_cited = set()
...
def do_get_citation_network(data, start, end):
    ...
    if len(graph) == 0:
        return 'There are no DOIs within the timespan with which to generate the graph \U0001F622'
    else:
        return graph 
